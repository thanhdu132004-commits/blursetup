...
export function AdminSidebar() {
  ...
  return (
    <aside className="w-64 bg-white dark:bg-[#18181b] border-r border-gray-200 dark:border-gray-800 flex flex-col hidden md:flex z-20 shadow-sm transition-colors duration-300">
      <div className="h-16 flex items-center px-6 border-b border-gray-100 dark:border-gray-800">
        <Link href="/admin" className="flex items-center gap-2">
          ...
          <span className="bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 text-[10px] font-bold px-2 py-0.5 rounded-full uppercase">Admin</span>
        </Link>
      </div>

      <nav className="flex-1 py-6 px-4 space-y-1.5">
        <div className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-4 px-2">Quản lý hệ thống</div>
        {menuItems.map((item) => (
          <Link
            key={item.href}
            href={item.href}
