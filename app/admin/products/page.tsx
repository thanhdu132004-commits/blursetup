...
function AdminProductsPage() {
  ...
  // Sinh ra template rỗng khi người dùng đổi category
  useEffect(() => {
  ...
  const handleEditClick = (product: any) => {
    ...
    template.forEach(key => { mergedSpecs[key] = existingSpecs[key] || ""; });
    Object.keys(existingSpecs).forEach(key => { if(!template.includes(key)) mergedSpecs[key] = existingSpecs[key]; });
  ...
  if (loading && products.length === 0) return <div className="p-8 dark:text-gray-400">Đang tải dữ liệu...</div>;

  return (
    <div className="p-8 bg-gray-50 dark:bg-[#09090b] min-h-screen transition-colors duration-300">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <h1 className="text-2xl font-black text-gray-900 dark:text-white">Quản lý sản phẩm</h1>
        <div className="flex w-full sm:w-auto items-center gap-3">
          <div className="relative flex-1 sm:w-64">
            <Search className="absolute left-3 top-2.5 w-4 h-4 text-gray-400" />
            <input type="text" placeholder="Tìm tên sản phẩm..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-9 pr-4 py-2 border dark:border-gray-700 rounded-xl text-sm outline-none focus:border-red-500 bg-white dark:bg-[#18181b] dark:text-white" />
          </div>
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
            {currentProducts.length > 0 ? currentProducts.map((p) => (
              <tr key={p.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                <td className="px-6 py-4 font-bold text-sm flex items-center gap-3 text-gray-900 dark:text-gray-100">
                    <img src={p.imageUrl || "/placeholder.jpg"} className="w-10 h-10 rounded-lg object-cover border dark:border-gray-700" alt={p.name} />
                    <div>
                ...
                <td className="px-6 py-4 text-sm font-semibold text-gray-700 dark:text-gray-300">{p.brand}</td>
                <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">{p.category}</td>
                <td className="px-6 py-4"><span className="font-black text-red-600 dark:text-red-500">{p.price.toLocaleString("vi-VN")} ₫</span></td>
              ...
              <tr><td colSpan={5} className="px-6 py-12 text-center text-gray-500 dark:text-gray-400">Không tìm thấy sản phẩm.</td></tr>
            )}
          </tbody>
        </table>
        {totalPages > 1 && (
          <div className="p-4 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between bg-gray-50/50 dark:bg-gray-900 transition-colors duration-300">
            ...
            <div className="flex gap-2">
              <button 
                ...
                className="p-1.5 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-[#18181b] disabled:opacity-50 transition-colors"
              >
                <ChevronLeft className="w-4 h-4 dark:text-gray-300" />
              </button>
              <span className="px-3 py-1.5 text-xs font-bold bg-white dark:bg-[#18181b] border border-gray-200 dark:border-gray-700 rounded-lg dark:text-white">
      ...
      {isDrawerOpen && (
        <div className="fixed inset-0 z-[100] flex justify-end">
          ...
          <div className="relative w-full max-w-[600px] bg-white dark:bg-[#18181b] h-full p-6 shadow-2xl flex flex-col animate-in slide-in-from-right duration-300 transition-colors duration-300">
            <div className="flex justify-between items-center mb-6 border-b border-gray-200 dark:border-gray-800 pb-4">
              <h2 className="text-xl font-black text-gray-900 dark:text-white">{editingId ? "Sửa sản phẩm" : "Thêm sản phẩm mới"}</h2>
              <button onClick={() => setIsDrawerOpen(false)} className="p-1.5 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-full"><X className="w-5 h-5 text-gray-500 dark:text-gray-400" /></button>
            </div>
            ...
            <div className="space-y-6 flex-1 overflow-y-auto pr-2 custom-scrollbar pb-10">
              <div className="space-y-4">
                <div className="space-y-1">
                  ...
                  <input className="w-full p-3 border border-gray-200 dark:border-gray-700 bg-white dark:bg-[#09090b] rounded-xl text-sm outline-none focus:border-red-500 dark:text-white" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
                </div>
                
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-gray-700 dark:text-gray-300">Danh mục (*)</label>
                    <select className="w-full p-3 border border-gray-200 dark:border-gray-700 rounded-xl text-sm outline-none focus:border-red-500 bg-white dark:bg-[#09090b] dark:text-white" value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})}>
                  ...
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-gray-700 dark:text-gray-300">Thương hiệu (*)</label>
                    <select className="w-full p-3 border border-gray-200 dark:border-gray-700 rounded-xl text-sm outline-none focus:border-red-500 bg-white dark:bg-[#09090b] dark:text-white" value={formData.brand} onChange={e => setFormData({...formData, brand: e.target.value})}>
                ...
                <div className="space-y-1">
                  <label className="text-xs font-bold text-gray-700 dark:text-gray-300">Tình trạng (*)</label>
                  <select className="w-full p-3 border border-gray-200 dark:border-gray-700 bg-white dark:bg-[#09090b] rounded-xl text-sm outline-none focus:border-red-500 dark:text-white" value={formData.condition} onChange={e => setFormData({...formData, condition: e.target.value})}>
              ...
              <div className="space-y-4">
                <div className="space-y-1"><label className="text-xs font-bold text-gray-700 dark:text-gray-300">Link ảnh chính</label><input className="w-full p-3 border border-gray-200 dark:border-gray-700 rounded-xl text-sm bg-white dark:bg-[#09090b] dark:text-white" value={formData.imageUrl} onChange={e => setFormData({...formData, imageUrl: e.target.value})} /></div>
                
                <div className="space-y-1">
                  <label className="text-xs font-bold text-gray-700 dark:text-gray-300">
                    Ảnh phụ (Gallery) <span className="text-[10px] text-gray-400 font-normal ml-1">(cách nhau dấu phẩy)</span>
                  </label>
                  <textarea rows={3} className="w-full p-3 border border-gray-200 dark:border-gray-700 rounded-xl text-sm bg-white dark:bg-[#09090b] dark:text-white resize-none" value={formData.gallery} onChange={e => setFormData({...formData, gallery: e.target.value})} />
                </div>

                <div className="space-y-1">
                  <label className="text-xs font-bold text-gray-700 dark:text-gray-300 flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-red-600"/> Tính năng nổi bật</label>
                  <textarea rows={4} className="w-full p-3 border border-gray-200 dark:border-gray-700 rounded-xl text-sm bg-white dark:bg-[#09090b] dark:text-white resize-none" value={formData.highlightsText} onChange={e => setFormData({...formData, highlightsText: e.target.value})} />
                </div>
                
                <div className="space-y-1">
                  <label className="text-xs font-bold text-gray-700 dark:text-gray-300">Bài viết / Thông tin chi tiết</label>
                  <textarea rows={5} className="w-full p-3 border border-gray-200 dark:border-gray-700 rounded-xl text-sm bg-white dark:bg-[#09090b] dark:text-white" value={formData.description || ""} onChange={e => setFormData({...formData, description: e.target.value})} />
                </div>
              </div>

              <div className="bg-red-50/50 dark:bg-red-900/10 p-4 rounded-xl border border-red-100 dark:border-red-900/30 space-y-4">
                <div className="flex items-center gap-2 border-b border-red-100 dark:border-red-900/30 pb-2">
                  ...
                  <h3 className="text-sm font-black text-gray-900 dark:text-white">Thông số kỹ thuật</h3>
                </div>
