...
function UsedProductsPage() {
  ...
  const sortedProducts = useMemo(() => {
    // Tạo một bản sao của mảng products để không làm ảnh hưởng mảng gốc
    let result = [...products]; 
    ...
    // Nếu là "newest" thì giữ nguyên vì Database (actions.ts) đã lấy mới nhất rồi
    ...
    <div className="bg-gray-50 min-h-screen pb-12">
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-[1200px] mx-auto px-4 md:px-8 py-3 flex items-center gap-2 text-xs font-medium text-gray-500">
          ...
          <span className="text-gray-900 font-bold">Hàng cũ giá tốt</span>
        </div>
      </div>
      ...
      <div className="max-w-[1200px] mx-auto px-4 md:px-8 py-8 space-y-8">
        ...
        {/* BANNER HÀNG CŨ */}
        <div className="bg-gradient-to-r from-gray-900 to-gray-800 rounded-2xl p-6 md:p-10 text-white shadow-lg flex flex-col md:flex-row items-center justify-between gap-6 overflow-hidden relative">
        ...
        <div className="flex items-center justify-between bg-white p-4 rounded-xl shadow-sm border border-gray-200">
          ...
          <select 
            ...
            className="border border-gray-200 text-sm font-medium text-gray-700 py-2 px-4 rounded-lg outline-none focus:border-[#d70018] cursor-pointer transition-colors"
          >
        ...
        {loading ? (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            {[1, 2, 3, 4, 5].map(i => (
              <div key={i} className="bg-gray-200 animate-pulse aspect-[3/4] rounded-xl" />
            ))}
          </div>
          ...
          <div className="bg-white rounded-2xl p-12 text-center border border-gray-200 shadow-sm flex flex-col items-center justify-center">
            <Laptop className="w-16 h-16 text-gray-300 mb-4" />
            <h3 className="text-lg font-bold text-gray-900 mb-2">Chưa có sản phẩm cũ nào</h3>
