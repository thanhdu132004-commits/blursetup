    ...
    <div className="bg-[#f4f6f8] min-h-screen pb-12">
      ...
      <div className="max-w-[1200px] mx-auto px-4 md:px-8">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-2xl font-black text-gray-900 uppercase">Sản phẩm {categoryName}</h1>
          <span className="text-sm font-bold text-[#d70018] bg-red-100 px-2.5 py-0.5 rounded-md border border-red-200">
        ...
        {loading ? (
          ...
          <div className="flex flex-col lg:flex-row gap-6 items-start">
            ...
            <div className="w-full lg:w-[260px] bg-white rounded-2xl shadow-sm border border-gray-100 p-5 flex-shrink-0 lg:sticky lg:top-24">
              <div className="flex items-center justify-between border-b border-gray-100 pb-3 mb-4">
                ...
                {(selectedBrands.length > 0 || selectedCondition || selectedPrice) && (
                  <button onClick={clearAllFilters} className="text-xs font-bold text-gray-400 hover:text-red-600 transition flex items-center">
              ...
              {/* Lọc Hãng (Động) */}
              <div className="mb-6">
                <h4 className="text-sm font-bold text-gray-800 mb-3">Thương hiệu</h4>
                  ...
                  {availableBrands.map(brand => (
                    <label key={brand} className="flex items-center gap-2.5 cursor-pointer group">
                      <input 
                        ...
                        className="w-4 h-4 text-red-600 border-gray-300 rounded focus:ring-red-500 accent-[#d70018] cursor-pointer"
                      />
                      <span className="text-sm font-medium text-gray-700 group-hover:text-red-600 transition-colors">{brand}</span>
                    </label>
                  ))}
                </div>
              </div>
              ...
              <div className="mb-6 border-t border-gray-100 pt-4">
                <h4 className="text-sm font-bold text-gray-800 mb-3">Khoảng giá</h4>
                  ...
                  {[
                    ...
                    <label key={price.val} className="flex items-center gap-2.5 cursor-pointer group">
                      <input 
                        ...
                        className="w-4 h-4 text-red-600 border-gray-300 focus:ring-red-500 accent-[#d70018] cursor-pointer"
                      />
                      <span className="text-sm font-medium text-gray-700 group-hover:text-red-600 transition-colors">{price.label}</span>
                    </label>
                  ))}
                </div>
              </div>
              ...
              <div className="border-t border-gray-100 pt-4">
                <h4 className="text-sm font-bold text-gray-800 mb-3">Tình trạng</h4>
                <div className="flex gap-2">
                  <button 
                    onClick={() => setSelectedCondition(selectedCondition === "Mới" ? null : "Mới")}
                    className={`flex-1 py-2 text-xs font-bold rounded-lg border transition-all ${
                      selectedCondition === "Mới" ? "border-red-600 bg-red-50 text-red-600 shadow-sm" : "border-gray-200 bg-gray-50 text-gray-600 hover:bg-gray-100"
                    }`}
                  >Hàng Mới</button>
                  <button 
                    onClick={() => setSelectedCondition(selectedCondition === "Cũ" ? null : "Cũ")}
                    className={`flex-1 py-2 text-xs font-bold rounded-lg border transition-all ${
                      selectedCondition === "Cũ" ? "border-red-600 bg-red-50 text-red-600 shadow-sm" : "border-gray-200 bg-gray-50 text-gray-600 hover:bg-gray-100"
                    }`}
                  >Hàng Cũ</button>
                </div>
              </div>
            ...
            <div className="flex-1 w-full">
              {displayedProducts.length > 0 ? (
                ...
                <div className="bg-white rounded-2xl p-12 text-center border border-gray-200 flex flex-col items-center">
                  <AlertCircle className="w-12 h-12 text-gray-300 mb-4" />
                  <h3 className="text-lg font-bold text-gray-900 mb-2">Không tìm thấy sản phẩm</h3>
                  <p className="text-gray-500 text-sm mb-6">Không có sản phẩm nào phù hợp với bộ lọc của bạn.</p>
          ...
          <div className="bg-white rounded-2xl p-12 text-center border border-gray-200 flex flex-col items-center shadow-sm">
            <AlertCircle className="w-12 h-12 text-gray-300 mb-4" />
            <h3 className="text-lg font-bold text-gray-900 mb-2">Chưa có sản phẩm nào</h3>
            <p className="text-gray-500 text-sm mb-6">Hiện tại danh mục {categoryName} đang được cập nhật sản phẩm.</p>
            <Link href="/" className="px-6 py-2.5 bg-gray-900 text-white rounded-lg font-bold text-sm hover:bg-black transition shadow-sm">
              Quay lại trang chủ
            </Link>
          </div>
        )}
